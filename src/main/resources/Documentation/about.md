The @PLUGIN@ plugin adds a separate channel for Gerrit to store test metadata
and view them on the Gerrit UI.  The metadata can be stored in the Gerrit
database or in a completely separate datastore.
...
at the comments and decipher which ones to pay attention to.  After a while
the CI reports become more like spam.  Usually human reviewers only
...
bots to report info to Gerrit.  It provides a separate datastore to store the
data and provides a set of SSH and REST endpoints to access to retrieve and
save the data.  It also provides UI components to view the data outside of the
