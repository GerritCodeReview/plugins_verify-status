...
public class JobsSummaryPanel extends FlowPanel {
    ...
    new RestApi("changes").id(decodedChangeId).view("revisions").id(rev.id())
        ...
        .get(new AsyncCallback<VerificationStats>() {
          ...
          public void onSuccess(VerificationStats result) {
            if (result != null) {
              int numJobs =
                  result.passed() + result.failed() + result.unstable();
              if (numJobs > 0) {
                display(result);
              }
            }
  ...
  private void display(VerificationStats results) {
    ...
    Label passed = new Label(Integer.toString(results.passed()));
    passed.getElement().getStyle().setColor(COLOR_GREEN);
    g.setWidget(1, 0, passed);
    Label failed = new Label(Integer.toString(results.failed()));
    failed.getElement().getStyle().setColor(COLOR_RED);
    g.setWidget(1, 1, failed);
    Label unstable = new Label(Integer.toString(results.unstable()));
