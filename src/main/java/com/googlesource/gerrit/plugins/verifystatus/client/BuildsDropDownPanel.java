...
public class BuildsDropDownPanel extends FlowPanel {
  ...
  private void display(Map<String, VerificationInfo> jobs) {
    ...
    for (Map.Entry<String, VerificationInfo> job : jobs.entrySet()) {
      ...
      // Mapping for votes and displayed results
      // -2=Failed(Voting), -1=Failed(Non-Voting), 0=Non-voting,
      // +1=Passed(Non-voting), +2=Passed(Voting)
      if (vote > 1) {
        ...
        p.add(new InlineHyperlink("Passed-NV", job.getValue().url()));
      } else if (vote < 0) {
        p.add(new Image(VerifyStatusPlugin.RESOURCES.info()));
        p.add(new InlineHyperlink("Failed-NV", job.getValue().url()));
      } else {
        p.add(new Image(VerifyStatusPlugin.RESOURCES.info()));
        p.add(new InlineHyperlink("Warning-NV", job.getValue().url()));
      }
