...
public class VerifyStatusPreferencesPanel extends VerticalPanel {
  static class Factory implements Panel.EntryPoint {
    @Override
    public void onLoad(Panel panel) {
      panel.setWidget(new VerifyStatusPreferencesPanel());
    }
  }
  ...
  private void display(PreferencesInfo info) {
    ...
    Label label = new Label("Number of jobs to show:");
    p.add(label);
    ...
    for (PreferencesInfo.VisibleJobs v :
      PreferencesInfo.VisibleJobs.values()) {
      ui.addItem(v.name(), v.name());
    }

    for (int i = 0; i < ui.getItemCount(); i++) {
      if (info.visibleJobs().name().equals(ui.getValue(i))) {
    ...
    ui.addChangeHandler(new ChangeHandler() {
      ...
      public void onChange(ChangeEvent event) {
        ...
        info.visibleJobs(ui.getSelectedValue());
        new RestApi("accounts").id("self")
            .view(Plugin.get().getPluginName(), "preferences")
