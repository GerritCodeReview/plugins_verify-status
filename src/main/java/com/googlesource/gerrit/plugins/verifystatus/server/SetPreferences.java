...
import com.googlesource.gerrit.plugins.verifystatus.server.GetPreferences.PreferencesInfo;
import com.googlesource.gerrit.plugins.verifystatus.server.SetPreferences.Input;
...
public class SetPreferences implements RestModifyView<AccountResource, Input> {
  ...
  public GetPreferences.PreferencesInfo apply(AccountResource rsrc, Input input)
    ...
    if (input.visibleJobs == null) {
      input.visibleJobs = VisibleJobs.ALL;
    }
    ...
    try {
      ...
      if (input.visibleJobs != VisibleJobs.ALL) {
        p.getConfig().setEnum("plugin", pluginName, "strategy",
            input.visibleJobs);
      } else {
    ...
    info.visibleJobs = input.visibleJobs;
    return info;
  }
}
